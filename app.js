...
function renderRecentPayments() {
  ...
  if (!payments.length) {
    document.getElementById('recent-payments').innerHTML = '<div class="empty-state"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Belum ada pembayaran</p></div>';
    return;
  }
...
async function renderCustomerTable() {
  ...
  if (!paged.length) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Tidak ada data</p></div></td></tr>`;
  } else {
  ...
  document.getElementById('custDetailBody').innerHTML = `
    ...
    <div id="detail-tabs">
      ...
      <div id="tab-history" class="tab-pane">
        ...
        `<div class="empty-state"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Belum ada pembayaran</p></div>`}
      </div>
      <!-- FOTO -->
...
function renderPaymentTable() {
  ...
  if (!paged.length) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Tidak ada data pembayaran</p></div></td></tr>`;
  } else {
...
async function renderKartuList() {
  ...
  if (!paged.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Tidak ada kartu angsuran</p></div>`;
  } else {
...
function renderPagination(containerId, current, total, onClick) {
  ...
  html += `<button class="page-btn" ${current===total?'disabled':''} onclick="pgGo('${containerId}',${current+1})">"º</button>`;
  el.innerHTML = html;
}
...
async function renderLogTab() {
  ...
  if (!logs.length) {
    document.getElementById('activity-log').innerHTML =
      '<div class="empty-state"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Belum ada aktivitas</p></div>';
    return;
  }
...
function buildMessage(templateKey, customer) {
  ...
  const footer = `\n\nInfo lebih lanjut hubungi kami.\nTerima kasih atas kepercayaannya.  Bisnis Kredit Ruli Rizki Ariyanto_`;
...
function waFillQuickInfo() {
  ...
  infoEl.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
      ...
      <div> <strong>${c.noHp || '"”'}</strong> ${phone ? 'âœ…' : 'âŒ Tidak valid'}</div>
...
async function renderWaLog() {
  ...
  if (!logs.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon" style="font-size:36px;opacity:.4;">—</div><p>Belum ada pesan terkirim</p></div>`;
    return;
  }
  ...
  return `
    <div class="item-gallery" id="item-gallery-${customerId}">
      <div class="item-gallery-main" style="position:relative;">
        ...
        <button class="item-gallery-nav next" onclick="galleryNav(${1},'${customerId}')" style="${photos.length<2?'display:none':''}">"º</button>
